...
const homeBreadcrumb = { title: 'Home', titleKey: 'breadcrumb.home', url: APP_DEFAULT_PATH, icon: HomeTwoToneIcon };
...
export default function Breadcrumbs({ data, divider = true, title, icons = false, sx, ...rest }) {
  ...
  function CoreBreadcrumb() {
    return (
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ alignItems: { xs: 'flex-start', sm: 'center' }, justifyContent: 'space-between', gap: 1 }}>
          ...
          {breadcrumbItems.length &&
            breadcrumbItems.map((item, index) => (
              <Typography
                ...
                key={index}
              >
                {icons && item.icon && <item.icon />} {item.titleKey ? t(item.titleKey) : item.title}
              </Typography>
            ))}
          {activeItem && (
            <Typography variant="subtitle2" color="primary" sx={{ fontSize: '0.75rem', ...(icons && activeItem.icon && flexStyle) }}>
              ...
              {activeItem.titleKey ? t(activeItem.titleKey) : activeItem.title}
            </Typography>
          )}
        </MuiBreadcrumbs>

        <Typography variant="h3" sx={{ fontWeight: 500, textAlign: { xs: 'left', sm: 'right' } }}>
          {title || (activeItem?.titleKey ? t(activeItem.titleKey) : activeItem?.title)}
        </Typography>
      </Stack>
    );
  }
