...
export default function Departments() {
  ...
  const [form, setForm] = useState({ department: '', status: 1 });
  const [errors, setErrors] = useState<Record<string, string>>({});

  const canCreate = hasPermission(user, 'hrms.departments.create');
  const canEdit = hasPermission(user, 'hrms.departments.edit');
  const canDelete = hasPermission(user, 'hrms.departments.delete');
  ...
  const exportTitle = `${t('masters.departments') || 'Departments'} Report`;
  const exportColumns = useMemo(
    () => [
      { key: '__sno', label: t('common.sno') || 'S.No.' },
      { key: 'department', label: tl('Department') },
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      if (modal.mode === 'edit') {
        await updateMaster({ type: 'departments', id: modal.row.id, data: form }).unwrap();
      } else {
        await createMaster({ type: 'departments', data: form }).unwrap();
      }
      dispatch(showNotification({ message: `${t('masters.department') || 'Department'} saved successfully.` }));
      handleCloseModal();
    } catch (error: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMaster({ type: 'departments', id: deleteRow.id }).unwrap();
      dispatch(showNotification({ message: `${t('masters.department') || 'Department'} deleted successfully.` }));
      setDeleteRow(null);
    } catch (error: any) {
  ...
  return (
    <Stack sx={{ gap: 2 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('masters.managePrefix')} {(t('masters.departments') || 'Departments').toLowerCase()} {t('masters.manageSuffix')}
          </Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 1, alignItems: { xs: 'stretch', sm: 'center' } }}>
          ...
          {canCreate && (
            <Button variant="contained" color="primary" startIcon={<AddOutlined />} onClick={handleOpenCreate} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}>
              {t('common.create')} {t('masters.department') || 'Department'}
            </Button>
          )}
        </Stack>
      </Stack>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 6 }}>
            <SearchTextField
              fullWidth
              size="small"
              label={`${t('common.search')} ${t('masters.departments') || 'Departments'}`}
      ...
      <MainCard
        title={`${t('masters.departments') || 'Departments'} (${totalRows})`}
      ...
      >
        <TableContainer sx={{ '&::-webkit-scrollbar': { height: 8 }, '&::-webkit-scrollbar-thumb': { bgcolor: 'divider', borderRadius: 8 } }}>
          <Table sx={{ minWidth: 600 }}>
            ...
            <TableBody>
              {rows.map((row, index) => (
                <TableRow key={`departments-${row.id}`} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                  ...
                  <TableCell align="right">
                    <Stack direction="row" sx={{ justifyContent: 'flex-end', gap: 0.5 }}>
                      {canEdit && (
                        <IconButton size="small" color="success" aria-label={`edit department`} onClick={() => handleOpenEdit(row)}>
                          <EditOutlined fontSize="small" />
                        </IconButton>
                      )}
                      {canDelete && (
                        <IconButton size="small" color="error" aria-label={`delete department`} onClick={() => setDeleteRow(row)}>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <DialogTitle component="div" sx={{ pb: 1 }}>
            <Typography variant="h3" component="h2">
              {modal.mode === 'edit' ? t('common.update') : t('common.create')} {t('masters.department') || 'Department'}
            </Typography>
          </DialogTitle>
      ...
      <Dialog open={Boolean(deleteRow)} onClose={() => setDeleteRow(null)} fullWidth maxWidth="xs">
        <DialogTitle>{t('common.delete')} {t('masters.department') || 'Department'}</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
