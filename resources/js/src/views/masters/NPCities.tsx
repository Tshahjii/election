...
export default function NPCities() {
  ...
  const exportTitle = `Nagar Panchayat Cities Report`;
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      if (modal.mode === 'edit') {
        await updateMaster({ type: 'np-cities', id: modal.row.id, data: form }).unwrap();
      } else {
        await createMaster({ type: 'np-cities', data: form }).unwrap();
      }
      dispatch(showNotification({ message: `Nagar Panchayat City saved successfully.` }));
      handleCloseModal();
    } catch (error: any) {
  ...
  const handleDownloadTemplate = () => {
    downloadCsvTemplate(`np-cities-import`, [
  ...
  const handleImportSubmit = async () => {
    ...
    try {
      const response = await importMaster({ type: 'np-cities', data: formData }).unwrap();
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMaster({ type: 'np-cities', id: deleteRow.id }).unwrap();
      dispatch(showNotification({ message: `Nagar Panchayat City deleted successfully.` }));
      setDeleteRow(null);
    } catch (error: any) {
  ...
  return (
    <Stack sx={{ gap: 2 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('masters.managePrefix')} Nagar Panchayat cities {t('masters.manageSuffix')}
          </Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 1, alignItems: { xs: 'stretch', sm: 'center' } }}>
          ...
          {canCreate && (
            <Button variant="contained" color="primary" startIcon={<AddOutlined />} onClick={handleOpenCreate} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}>
              {t('common.create')} Nagar Panchayat City
            </Button>
          )}
        </Stack>
      </Stack>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 3 }}>
            <SearchTextField
              fullWidth
              size="small"
              label={`${t('common.search')} Nagar Panchayat Cities`}
      ...
      <MainCard
        title={`Nagar Panchayat Cities (${totalRows})`}
      ...
      >
        <TableContainer sx={{ '&::-webkit-scrollbar': { height: 8 }, '&::-webkit-scrollbar-thumb': { bgcolor: 'divider', borderRadius: 8 } }}>
          <Table sx={{ minWidth: 800 }}>
            ...
            <TableBody>
              {rows.map((row, index) => (
                <TableRow key={`np-cities-${row.id}`} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <DialogTitle component="div" sx={{ pb: 1 }}>
            <Typography variant="h3" component="h2">
              {modal.mode === 'edit' ? t('common.update') : t('common.create')} Nagar Panchayat City
            </Typography>
          </DialogTitle>
      ...
      <Dialog open={Boolean(deleteRow)} onClose={() => setDeleteRow(null)} fullWidth maxWidth="xs">
        <DialogTitle>{t('common.delete')} Nagar Panchayat City</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
