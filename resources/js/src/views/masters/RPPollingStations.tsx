...
export default function RPPollingStations() {
  ...
  const options = useMemo(() => {
    return optionsData || { states: [], districts: [], rp_cities: [], rp_wards: [] };
  }, [optionsData]);

  const debouncedFilters = useDebounce(filters, 400);

  const { data: listData, isFetching: loading } = useGetMastersQuery({
    type: 'rp-polling-stations',
    params: {
  ...
  const filterWardOptions = useMemo(() => {
    if (!filters.city_id) return options.rp_wards || [];
    return (options.rp_wards || []).filter((w: any) => Number(w.city_id) === Number(filters.city_id));
  }, [filters.city_id, options.rp_wards]);
  ...
  const formWardOptions = useMemo(() => {
    ...
    return (options.rp_wards || []).filter((w: any) => Number(w.city_id) === Number(form.city_id));
  }, [form.city_id, options.rp_wards]);
  ...
  const exportTitle = `Nagri Nikay Polling Stations Report`;
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      if (modal.mode === 'edit') {
        await updateMaster({ type: 'rp-polling-stations', id: modal.row.id, data: form }).unwrap();
      } else {
        await createMaster({ type: 'rp-polling-stations', data: form }).unwrap();
      }
      dispatch(showNotification({ message: `Nagri Nikay Polling Station saved successfully.` }));
      handleCloseModal();
    } catch (error: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMaster({ type: 'rp-polling-stations', id: deleteRow.id }).unwrap();
      dispatch(showNotification({ message: `Nagri Nikay Polling Station deleted successfully.` }));
      setDeleteRow(null);
    } catch (error: any) {
  ...
  return (
    <Stack sx={{ gap: 2 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('masters.managePrefix')} Nagri Nikay polling stations {t('masters.manageSuffix')}
          </Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 1, alignItems: { xs: 'stretch', sm: 'center' } }}>
          ...
          {canCreate && (
            <Button variant="contained" color="primary" startIcon={<AddOutlined />} onClick={handleOpenCreate} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}>
              {t('common.create')} Nagri Nikay Polling Station
            </Button>
          )}
        </Stack>
      </Stack>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 3 }}>
            <SearchTextField
              fullWidth
              size="small"
              label={`${t('common.search')} Nagri Nikay Polling Stations`}
      ...
      <MainCard
        title={`Nagri Nikay Polling Stations (${totalRows})`}
      ...
      >
        <TableContainer sx={{ '&::-webkit-scrollbar': { height: 8 }, '&::-webkit-scrollbar-thumb': { bgcolor: 'divider', borderRadius: 8 } }}>
          <Table sx={{ minWidth: 960 }}>
            ...
            <TableBody>
              {rows.map((row, index) => (
                <TableRow key={`rp-polling-stations-${row.id}`} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <DialogTitle component="div" sx={{ pb: 1 }}>
            <Typography variant="h3" component="h2">
              {modal.mode === 'edit' ? t('common.update') : t('common.create')} Nagri Nikay Polling Station
            </Typography>
          </DialogTitle>
      ...
      <Dialog open={Boolean(deleteRow)} onClose={() => setDeleteRow(null)} fullWidth maxWidth="xs">
        <DialogTitle>{t('common.delete')} Nagri Nikay Polling Station</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
