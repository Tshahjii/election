...
export default function Countries() {
  ...
  const canCreate = hasPermission(user, 'masters.countries.create');
  const canEdit = hasPermission(user, 'masters.countries.edit');
  const canDelete = hasPermission(user, 'masters.countries.delete');
  ...
  const rows = useMemo(() => {
    ...
    return rowsData.slice().sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  }, [listData]);

  const totalRows = listData?.total || 0;

  const exportTitle = `${t('masters.countries') || 'Countries'} Report`;
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (!form.name || form.name.trim() === '') {
      clientErrors.name = 'Country Name is required.';
    }
    ...
    try {
      if (modal.mode === 'edit') {
        await updateMaster({ type: 'countries', id: modal.row.id, data: formData }).unwrap();
      } else {
        await createMaster({ type: 'countries', data: formData }).unwrap();
      }
      dispatch(showNotification({ message: `${t('masters.country') || 'Country'} saved successfully.` }));
      handleCloseModal();
    } catch (error: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMaster({ type: 'countries', id: deleteRow.id }).unwrap();
      dispatch(showNotification({ message: `${t('masters.country') || 'Country'} deleted successfully.` }));
      setDeleteRow(null);
    } catch (error: any) {
  ...
  return (
    <Stack sx={{ gap: 2 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('masters.managePrefix')} {(t('masters.countries') || 'Countries').toLowerCase()} {t('masters.manageSuffix')}
          </Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 1, alignItems: { xs: 'stretch', sm: 'center' } }}>
          ...
          {canCreate && (
            <Button variant="contained" color="primary" startIcon={<AddOutlined />} onClick={handleOpenCreate} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}>
              {t('common.create')} {t('masters.country') || 'Country'}
            </Button>
          )}
        </Stack>
      </Stack>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 6 }}>
            <SearchTextField
              fullWidth
              size="small"
              label={`${t('common.search')} ${t('masters.countries') || 'Countries'}`}
      ...
      <MainCard
        title={`${t('masters.countries') || 'Countries'} (${totalRows})`}
      ...
      >
        <TableContainer sx={{ '&::-webkit-scrollbar': { height: 8 }, '&::-webkit-scrollbar-thumb': { bgcolor: 'divider', borderRadius: 8 } }}>
          <Table sx={{ minWidth: 600 }}>
            ...
            <TableBody>
              {rows.map((row, index) => (
                <TableRow key={`countries-${row.id}`} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                  ...
                  <TableCell align="right">
                    <Stack direction="row" sx={{ justifyContent: 'flex-end', gap: 0.5 }}>
                      {canEdit && (
                        <IconButton size="small" color="success" aria-label={`edit ${t('masters.country') || 'Country'}`} onClick={() => handleOpenEdit(row)}>
                          <EditOutlined fontSize="small" />
                        </IconButton>
                      )}
                      {canDelete && (
                        <IconButton size="small" color="error" aria-label={`delete ${t('masters.country') || 'Country'}`} onClick={() => setDeleteRow(row)}>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <DialogTitle component="div" sx={{ pb: 1 }}>
            <Typography variant="h3" component="h2">
              {modal.mode === 'edit' ? t('common.update') : t('common.create')} {t('masters.country') || 'Country'}
            </Typography>
          </DialogTitle>
      ...
      <Dialog open={Boolean(deleteRow)} onClose={() => setDeleteRow(null)} fullWidth maxWidth="xs">
        <DialogTitle>{t('common.delete')} {t('masters.country') || 'Country'}</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
