...
export default function EmpTypes() {
  ...
  const [form, setForm] = useState({ emp_type: '', status: 1 });
  const [errors, setErrors] = useState<Record<string, string>>({});

  const canCreate = hasPermission(user, 'hrms.emp_types.create');
  const canEdit = hasPermission(user, 'hrms.emp_types.edit');
  const canDelete = hasPermission(user, 'hrms.emp_types.delete');
  ...
  const rows = useMemo(() => {
    const rowsData = listData?.data || [];
    return rowsData.slice().sort((a: any, b: any) => Number(b.id) - Number(a.id));
  }, [listData]);

  const totalRows = listData?.total || 0;

  const exportTitle = `${t('masters.employeeTypes') || 'Employee Types'} Report`;
  const exportColumns = useMemo(
    () => [
      { key: '__sno', label: t('common.sno') || 'S.No.' },
      { key: 'emp_type', label: tl('Employee Type') },
  ...
  const handleGetRows = async () => {
    ...
    const sorted = rawRows.slice().sort((a: any, b: any) => Number(b.id) - Number(a.id));
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (!form.emp_type || form.emp_type.trim() === '') {
      clientErrors.emp_type = 'Employee Type is required.';
    }
    ...
    try {
      if (modal.mode === 'edit') {
        await updateMaster({ type: 'emp-types', id: modal.row.id, data: form }).unwrap();
      } else {
        await createMaster({ type: 'emp-types', data: form }).unwrap();
      }
      dispatch(showNotification({ message: `${t('masters.employeeType') || 'Employee Type'} saved successfully.` }));
      handleCloseModal();
    } catch (error: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMaster({ type: 'emp-types', id: deleteRow.id }).unwrap();
      dispatch(showNotification({ message: `${t('masters.employeeType') || 'Employee Type'} deleted successfully.` }));
      setDeleteRow(null);
    } catch (error: any) {
  ...
  return (
    <Stack sx={{ gap: 2 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('masters.managePrefix')} {(t('masters.employeeTypes') || 'Employee Types').toLowerCase()} {t('masters.manageSuffix')}
          </Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 1, alignItems: { xs: 'stretch', sm: 'center' } }}>
          ...
          {canCreate && (
            <Button variant="contained" color="primary" startIcon={<AddOutlined />} onClick={handleOpenCreate} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}>
              {t('common.create')} {t('masters.employeeType') || 'Employee Type'}
            </Button>
          )}
        </Stack>
      </Stack>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 6 }}>
            <SearchTextField
              fullWidth
              size="small"
              label={`${t('common.search')} ${t('masters.employeeTypes') || 'Employee Types'}`}
      ...
      <MainCard
        title={`${t('masters.employeeTypes') || 'Employee Types'} (${totalRows})`}
      ...
      >
        <TableContainer sx={{ '&::-webkit-scrollbar': { height: 8 }, '&::-webkit-scrollbar-thumb': { bgcolor: 'divider', borderRadius: 8 } }}>
          <Table sx={{ minWidth: 600 }}>
            ...
            <TableBody>
              {rows.map((row, index) => (
                <TableRow key={`emp-types-${row.id}`} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                  ...
                  <TableCell align="right">
                    <Stack direction="row" sx={{ justifyContent: 'flex-end', gap: 0.5 }}>
                      {canEdit && (
                        <IconButton size="small" color="success" aria-label={`edit ${t('masters.employeeType') || 'Employee Type'}`} onClick={() => handleOpenEdit(row)}>
                          <EditOutlined fontSize="small" />
                        </IconButton>
                      )}
                      {canDelete && (
                        <IconButton size="small" color="error" aria-label={`delete ${t('masters.employeeType') || 'Employee Type'}`} onClick={() => setDeleteRow(row)}>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <DialogTitle component="div" sx={{ pb: 1 }}>
            <Typography variant="h3" component="h2">
              {modal.mode === 'edit' ? t('common.update') : t('common.create')} {t('masters.employeeType') || 'Employee Type'}
            </Typography>
          </DialogTitle>
            ...
            <Grid container spacing={2} sx={{ pt: 0.5 }}>
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  ...
                  value={form.emp_type}
                  onChange={handleFormChange('emp_type')}
                  error={!!errors.emp_type}
                  helperText={errors.emp_type}
                  size="small"
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
      ...
      <Dialog open={Boolean(deleteRow)} onClose={() => setDeleteRow(null)} fullWidth maxWidth="xs">
        <DialogTitle>{t('common.delete')} {t('masters.employeeType') || 'Employee Type'}</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
