...
export default function RPWards() {
  const dispatch = useDispatch();
  const { t, tl } = useAppPreferences();
  const { user } = useSelector((state: any) => state.auth);

  const [filters, setFilters] = useState({ search: '', status: '', state_id: '', district_id: '', city_id: '' });
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [modal, setModal] = useState({ open: false, mode: 'create', row: null });
  const [deleteRow, setDeleteRow] = useState(null);
  const [form, setForm] = useState<Record<string, any>>({ state_id: '', district_id: '', city_id: '', ward_no: '', ward_name: '', status: 1 });
  const [errors, setErrors] = useState<Record<string, string>>({});

  const canCreate = hasPermission(user, 'masters.wards.create');
  const canEdit = hasPermission(user, 'masters.wards.edit');
  const canDelete = hasPermission(user, 'masters.wards.delete');

  const { data: optionsData } = useGetOptionsQuery();
  const options = useMemo(() => {
    return optionsData || { states: [], districts: [], rp_cities: [] };
  }, [optionsData]);
  ...
  const filterCityOptions = useMemo(() => {
    if (!filters.district_id) return options.rp_cities || [];
    return (options.rp_cities || []).filter((c: any) => Number(c.district_id) === Number(filters.district_id));
  }, [filters.district_id, options.rp_cities]);
  ...
  const formCityOptions = useMemo(() => {
    ...
    return (options.rp_cities || []).filter((c: any) => Number(c.district_id) === Number(form.district_id));
  }, [form.district_id, options.rp_cities]);
  ...
  const exportTitle = `Nagri Nikay Wards Report`;
  const exportColumns = useMemo(
    () => [
      { key: '__sno', label: t('common.sno') || 'S.No.' },
      { key: 'ward_no', label: tl('Ward No') },
      { key: 'ward_name', label: tl('Ward Name') },
  ...
  const handleOpenCreate = () => {
    const next: Record<string, any> = { state_id: '', district_id: '', city_id: '', ward_no: '', ward_name: '', status: 1 };
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (!form.ward_no || String(form.ward_no).trim() === '') clientErrors.ward_no = 'Ward No is required.';
    if (!form.ward_name || form.ward_name.trim() === '') clientErrors.ward_name = 'Ward Name is required.';
    ...
    try {
      if (modal.mode === 'edit') {
        await updateMaster({ type: 'rp-wards', id: modal.row.id, data: form }).unwrap();
      } else {
        await createMaster({ type: 'rp-wards', data: form }).unwrap();
      }
      dispatch(showNotification({ message: `Nagri Nikay Ward saved successfully.` }));
      handleCloseModal();
    } catch (error: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMaster({ type: 'rp-wards', id: deleteRow.id }).unwrap();
      dispatch(showNotification({ message: `Nagri Nikay Ward deleted successfully.` }));
      setDeleteRow(null);
    } catch (error: any) {
  ...
  return (
    <Stack sx={{ gap: 2 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('masters.managePrefix')} Nagri Nikay wards {t('masters.manageSuffix')}
          </Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 1, alignItems: { xs: 'stretch', sm: 'center' } }}>
          ...
          {canCreate && (
            <Button variant="contained" color="primary" startIcon={<AddOutlined />} onClick={handleOpenCreate} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}>
              {t('common.create')} Nagri Nikay Ward
            </Button>
          )}
        </Stack>
      </Stack>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 3 }}>
            <SearchTextField
              fullWidth
              size="small"
              label={`${t('common.search')} Nagri Nikay Wards`}
          ...
          <Grid size={{ xs: 12, md: 2 }}>
            <FormControl fullWidth>
              <ChosenSelect
                ...
                onChange={(event) => {
                  setFilters((current) => ({ ...current, state_id: event.target.value, district_id: '', city_id: '' }));
                  setPage(1);
                }}
              />
            </FormControl>
          ...
          <Grid size={{ xs: 12, md: 2 }}>
            <FormControl fullWidth>
              <ChosenSelect
                ...
                onChange={(event) => {
                  setFilters((current) => ({ ...current, district_id: event.target.value, city_id: '' }));
                  setPage(1);
                }}
              />
            </FormControl>
          ...
          <Grid size={{ xs: 12, md: 2 }}>
            <FormControl fullWidth>
              <ChosenSelect
                ...
                onChange={(event) => {
                  setFilters((current) => ({ ...current, city_id: event.target.value }));
                  setPage(1);
                }}
              />
            </FormControl>
      ...
      <MainCard
        title={`Nagri Nikay Wards (${totalRows})`}
          ...
          <Table sx={{ minWidth: 800 }}>
            <TableHead>
              <TableRow sx={{ bgcolor: 'bg.100' }}>
                ...
                <TableCell sx={{ fontWeight: 800, whiteSpace: 'nowrap' }}>Ward No</TableCell>
                <TableCell sx={{ fontWeight: 800, whiteSpace: 'nowrap' }}>Ward Name</TableCell>
            ...
            <TableBody>
              {rows.map((row, index) => (
                <TableRow key={`rp-wards-${row.id}`} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                  ...
                  <TableCell align="right">
                    <Stack direction="row" sx={{ justifyContent: 'flex-end', gap: 0.5 }}>
                      {canEdit && (
                        <IconButton size="small" color="success" aria-label={`edit ward`} onClick={() => handleOpenEdit(row)}>
                          <EditOutlined fontSize="small" />
                        </IconButton>
                      )}
                      {canDelete && (
                        <IconButton size="small" color="error" aria-label={`delete ward`} onClick={() => setDeleteRow(row)}>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <DialogTitle component="div" sx={{ pb: 1 }}>
            <Typography variant="h3" component="h2">
              {modal.mode === 'edit' ? t('common.update') : t('common.create')} Nagri Nikay Ward
            </Typography>
          </DialogTitle>
            ...
            <Grid container spacing={2} sx={{ pt: 0.5 }}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  ...
                  label={tl('Ward No')}
                  value={form.ward_no}
                  onChange={handleFormChange('ward_no')}
                  error={!!errors.ward_no}
                  helperText={errors.ward_no}
                  size="small"
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  ...
                  onChange={handleFormChange('ward_name')}
                  error={!!errors.ward_name}
                  helperText={errors.ward_name}
                  size="small"
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
      ...
      <Dialog open={Boolean(deleteRow)} onClose={() => setDeleteRow(null)} fullWidth maxWidth="xs">
        <DialogTitle>{t('common.delete')} Nagri Nikay Ward</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
