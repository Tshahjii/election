...
export default function States() {
  ...
  const [filters, setFilters] = useState({ search: '', status: '', country_id: '' });
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [modal, setModal] = useState({ open: false, mode: 'create', row: null });
  const [deleteRow, setDeleteRow] = useState(null);
  const [form, setForm] = useState({ country_id: '', name: '', state_code: '', status: 1 });
  const [attachment, setAttachment] = useState<File | null>(null);
  const [errors, setErrors] = useState<Record<string, string>>({});

  const canCreate = hasPermission(user, 'masters.states.create');
  const canEdit = hasPermission(user, 'masters.states.edit');
  const canDelete = hasPermission(user, 'masters.states.delete');

  const { data: optionsData } = useGetOptionsQuery();
  const options = useMemo(() => optionsData || { countries: [] }, [optionsData]);
  ...
  const exportTitle = `${t('masters.states') || 'States'} Report`;
  const exportColumns = useMemo(
    () => [
      { key: '__sno', label: t('common.sno') || 'S.No.' },
      { key: 'name', label: tl('Name') },
      { key: 'state_code', label: tl('Code') },
  ...
  const handleOpenCreate = () => {
    ...
    setForm({ country_id: defaultCountry, name: '', state_code: '', status: 1 });
    setAttachment(null);
    setErrors({});
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (!form.name || form.name.trim() === '') {
      clientErrors.name = 'State Name is required.';
    }
    ...
    formData.append('state_code', form.state_code);
    ...
    try {
      if (modal.mode === 'edit') {
        await updateMaster({ type: 'states', id: modal.row.id, data: formData }).unwrap();
      } else {
        await createMaster({ type: 'states', data: formData }).unwrap();
      }
      dispatch(showNotification({ message: `${t('masters.state') || 'State'} saved successfully.` }));
      handleCloseModal();
      if (modal.mode === 'edit') {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMaster({ type: 'states', id: deleteRow.id }).unwrap();
      dispatch(showNotification({ message: `${t('masters.state') || 'State'} deleted successfully.` }));
      setDeleteRow(null);
    } catch (error: any) {
  ...
  return (
    <Stack sx={{ gap: 2 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('masters.managePrefix')} {(t('masters.states') || 'States').toLowerCase()} {t('masters.manageSuffix')}
          </Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 1, alignItems: { xs: 'stretch', sm: 'center' } }}>
          ...
          {canCreate && (
            <Button variant="contained" color="primary" startIcon={<AddOutlined />} onClick={handleOpenCreate} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}>
              {t('common.create')} {t('masters.state') || 'State'}
            </Button>
          )}
        </Stack>
      </Stack>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 4 }}>
            <SearchTextField
              fullWidth
              size="small"
              label={`${t('common.search')} ${t('masters.states') || 'States'}`}
      ...
      <MainCard
        title={`${t('masters.states') || 'States'} (${totalRows})`}
      ...
      >
        <TableContainer sx={{ '&::-webkit-scrollbar': { height: 8 }, '&::-webkit-scrollbar-thumb': { bgcolor: 'divider', borderRadius: 8 } }}>
          <Table sx={{ minWidth: 600 }}>
            ...
            <TableBody>
              {rows.map((row, index) => (
                <TableRow key={`states-${row.id}`} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                  ...
                  <TableCell align="right">
                    <Stack direction="row" sx={{ justifyContent: 'flex-end', gap: 0.5 }}>
                      {canEdit && (
                        <IconButton size="small" color="success" aria-label={`edit ${t('masters.state') || 'State'}`} onClick={() => handleOpenEdit(row)}>
                          <EditOutlined fontSize="small" />
                        </IconButton>
                      )}
                      {canDelete && (
                        <IconButton size="small" color="error" aria-label={`delete ${t('masters.state') || 'State'}`} onClick={() => setDeleteRow(row)}>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <DialogTitle component="div" sx={{ pb: 1 }}>
            <Typography variant="h3" component="h2">
              {modal.mode === 'edit' ? t('common.update') : t('common.create')} {t('masters.state') || 'State'}
            </Typography>
          </DialogTitle>
            ...
            <Grid container spacing={2} sx={{ pt: 0.5 }}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  ...
                  onChange={handleFormChange('state_code')}
      ...
      <Dialog open={Boolean(deleteRow)} onClose={() => setDeleteRow(null)} fullWidth maxWidth="xs">
        <DialogTitle>{t('common.delete')} {t('masters.state') || 'State'}</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
